...
export default function ShieldModal({
  ...
  const executeTransaction = async () => {
    ...
    const toastId = toast.loading(
      mode === "shield"
        ? `Shielding ${selectedSymbol}...`
        : `Unshielding ${selectedSymbol}...`,
    );
    ...
    try {
      ...
      if (!currentToken?.rate) {
        throw new Error("Token rate is missing or invalid");
      }
      ...
      if (mode === "shield") {
        ...
        const op = await accountInstance.withdraw({
          sender: address!,
          amount: tongoUnits, // Use calculated tongoUnits (amountWei / rate)
          to: address!,
        });

        // Unshielding requires a transaction from the main wallet to execute the withdraw op
            ...
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-3 py-1 bg-gray-200 hover:bg-gray-300 rounded-lg text-xs font-bold text-gray-600 transition-colors"
                >
                  25%
                </button>
                <button
                ...
                <button
                  ...
                  className="px-3 py-1 bg-black hover:bg-gray-800 rounded-lg text-xs font-bold text-white transition-colors"
                >
                  Max
                </button>
              </div>
